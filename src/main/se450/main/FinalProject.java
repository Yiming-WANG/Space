...
/**
 * The main class of this project. Handles the build, start and part of display (except the display of the game content) of the java gui window.
 * @author Yiming Wang
 *
 */
public class FinalProject extends JFrame implements IObservable, IScoreObservable
{
	...
	/**
	 * Handles the build, start and part of display (except the display of the game content) of the java gui window.
	 */
	public FinalProject()
	{
		ConfigurationParser.loadConfiguration("configuration.json");
		setSize(ConfigurationManager.getConfigurationManager().getConfiguration().getWidth(), ConfigurationManager.getConfigurationManager().getConfiguration().getHeight());//set the size of the window to 1440 x 840
	...
	/**
	 * the main entry point for the application which would create a new final project appllication.
	 * @param args
